...
OUTPUT_FIELDS = ["Название компании", "Чем занимается", "Категория",
    "Рубрика","Электронный адрес","Номер телефона", "Адрес сайта",
    "Регион", "Адрес офиса"]
...
PRIORITY_EMAIL_PREFIXES = ("info@", "sales@", "office@", "mail@", "admin@", "hello@", "contact@")
...
class Parser2GISPipe:
    ...
    def __init__(self, data_dir: Path = DATA_DIR, output_file: Path = OUTPUT_FILE):
        self.data_dir = data_dir
        self.output_file = output_file
        if os.path.exists(self.output_file):
            self.initial_df = pl.read_csv(self.output_file)
    ...
    def process_region(self) -> "Parser2GISPipe":
        ...
        def _join_region(row: dict) -> str | None:
    ...
    def process_address(self) -> "Parser2GISPipe":
        ...
        def _join_address(row: dict) -> str | None:
            parts = []
    ...
    def drop_no_contacts(self) -> "Parser2GISPipe":
        ...
        self._df = self._df.filter(
            pl.col("Электронный адрес").is_not_null()
    ...
    def deduplicate_contacts(self) -> "Parser2GISPipe":
        ...
        self._df = self._df.unique(subset=["Электронный адрес", "Номер телефона"], keep="first")
        return self
    ...
    def run(self) -> pl.DataFrame:
        ...
        print(f"Written {len(result)} rows to {self.output_file}")
        return result
