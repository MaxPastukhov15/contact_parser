...
def parse_arguments() -> tuple[argparse.Namespace, Configuration]:
    ...
    main_parser.add_argument('-i', '--url', nargs='+', default=None, required=True, help='URL с выдачей')
    main_parser.add_argument('-o', '--output-path', metavar='PATH', default=None, required=True, help='Путь до результирующего файла')
    main_parser.add_argument('-f', '--format', metavar='{csv,xlsx,json}', choices=['csv', 'xlsx', 'json'], default=None, required=True, help='Формат результирующего файла')
