...
from src.configs.settings import DataPaths, config
...
class TwoGisAPIParser:
    def __init__(self):
        ...
        self.regions = (
            pl.read_csv(DataPaths.REGIONS_PATH)
        ...
        self.rubrics = (
            pl.read_json(DataPaths.RUBRICS_PATH).select(pl.col("id")).to_series().to_list()
        )
    ...
    def parse_to_json(
        ...
        if response.status_code == 200:
            ...
            logger.debug(
                f"Successfully! Companies found on the page: {len(data)} (Total by request: {total_count})"
            )
            ...
            logger.warning(
                f"No data found (404) for the region {region_id}, query: '{query}', page: {page}"
            )
        else:
    ...
    def get_items(self, number_of_pages_to_parse: int = 1) -> None:
        if not self.rubrics or not self.regions:
            logger.error(
                "The lists of categories or regions are empty! Check the input CSV/JSON files."
            )
            return
